    ...
    <div className="mx-auto max-w-3xl px-4 py-8 md:px-8">
      <div className="flex items-center gap-4 mb-6">
        <div className="relative h-24 w-24 overflow-hidden rounded-full bg-ink/5 shrink-0">
          {player.photo_url && <Image src={player.photo_url} alt={player.name} fill sizes="96px" className="object-cover" />}
        </div>
        <div>
            ...
            {history.map((h: any) => (
              <tr key={h.id} className="border-b border-line last:border-0">
                ...
                <td className="px-3 py-2">
                  {h.match.is_home ? h.match.away_team?.name : h.match.home_team?.name}
                </td>
