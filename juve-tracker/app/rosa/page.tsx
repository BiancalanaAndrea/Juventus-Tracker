                ...
                {group.map((p: any) => (
                  ...
                  >
                    <div className="relative h-20 w-20 mx-auto mb-2 overflow-hidden rounded-full bg-ink/5">
                      {p.photo_url && <Image src={p.photo_url} alt={p.name} fill sizes="80px" className="object-cover" />}
                    </div>
                    ...
                    <div className="flex justify-center gap-3 text-xs text-steel">
                      ...
                      {pos === "GK" ? <span>{p.season_stats.goals_conceded} GS</span> : <span>{p.season_stats.goals} G</span>}
                    </div>
                  </Link>
                ))}
              </div>
