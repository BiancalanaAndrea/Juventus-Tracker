...
export async function POST() {
  try {
    ...
    // Current Juventus roster.
    const rosterResponse = await espn(`/all/teams/${TEAM_ID}/roster`);
    const roster = collectRosterPlayers(rosterResponse);

    // Current injury report, when ESPN exposes it.
    let injuries: any[] = [];
    try {
      const injuryResponse = await espn(`/all/teams/${TEAM_ID}/injuries`);
    ...
    // ESPN's all-competitions schedule is public and does not require a token.
    const schedule = await espn(`/all/teams/${TEAM_ID}/schedule?fixture=true`);
    const events = arr(schedule?.events).filter((e: any) => inWindow(e?.date, from, to));
    const uniqueEvents = [...new Map(events.map((e: any) => [String(e.id), e])).values()];

    let saved = 0;
    for (const event of uniqueEvents) {
      const leagueSlug = event?.competitions?.[0]?.league?.slug || 'all';
      let summary: any = null;
      try {
        summary = await espn(`/all/summary?event=${encodeURIComponent(event.id)}`);
      } catch {
