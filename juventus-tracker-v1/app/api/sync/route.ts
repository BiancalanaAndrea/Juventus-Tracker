...
function normalizeStats(block: any): any[] {
  ...
  if (direct.length && direct.some((x: any) => typeof x === 'object')) return direct;
  const keys = arr(block?.keys);
  const values = arr(block?.values);
  const names = arr(block?.names);
  if (keys.length && values.length) {
    return keys.map((k: any, i: number) => ({ key: String(k), name: String(k), displayName: names[i] || k, value: values[i] }));
  }
  return direct;
}
function statsArray(obj: any) {
  const source = obj?.statistics || obj?.stats || obj?.teamStats || obj?.athlete?.statistics;
  if (Array.isArray(source)) {
    const out: any[] = [];
    for (const block of source) out.push(...normalizeStats(block));
    return out;
  }
  return normalizeStats(source);
}
function playerPhoto(athlete: any) {
  const direct = athlete?.headshot?.href || athlete?.headshot?.url || athlete?.images?.[0]?.href || null;
  return direct || (athlete?.id ? `https://a.espncdn.com/i/headshots/soccer/players/full/${athlete.id}.png` : null);
}
...
export async function POST() {
  try {
    ...
    const coaches = arr(teamResponse?.team?.coaches || teamResponse?.coaches || teamResponse?.team?.coach);
    if (coaches.length) {
    ...
    // Use the competition scoreboards for the full season window. This is more reliable
    // than the team schedule feed, which can omit early fixtures on some ESPN mirrors.
    const eventMap = new Map<string, any>();
    const splitYear = Number(to.slice(0, 4));
    const ranges = [`${from.replaceAll('-', '')}-${splitYear - 1}1231`, `${splitYear}0101-${to.replaceAll('-', '')}`];
    for (const competition of COMPETITIONS) {
      for (const range of ranges) {
        try {
          const board = await espn(`/${competition.slug}/scoreboard?dates=${range}`);
          for (const e of arr(board?.events)) {
            if (!inWindow(e?.date, from, to)) continue;
            const teams = arr(e?.competitions?.[0]?.competitors);
            if (!teams.some((c: any) => String(c?.team?.id) === TEAM_ID)) continue;
            eventMap.set(String(e.id), { ...e, __leagueSlug: competition.slug });
          }
        } catch {
          // A competition may have no feed for this date range.
        }
      }
    }
    ...
    for (const event of uniqueEvents) {
      ...
      for (const mp of matchPlayers) {
        ...
        await supabase.from('player_match_stats').upsert({
          ...
          successful_passes: Math.round(playerMetric(stats, ['successful passes', 'passes completed'])),
          pass_accuracy: playerMetric(stats, ['pass accuracy', 'passing accuracy']),
          tackles: Math.round(playerMetric(stats, ['tackles'])),
          interceptions: Math.round(playerMetric(stats, ['interceptions'])),
          fouls: Math.round(playerMetric(stats, ['fouls'])),
