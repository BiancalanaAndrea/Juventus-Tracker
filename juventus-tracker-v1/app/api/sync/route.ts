...
export async function POST(){
 try{
  ...
  const team=await sm(`/teams/${TEAM_ID}?include=sidelined.type;sidelined.player;sidelined.sideline`,token);
  ...
  for(const brief of unique){
    ...
    for(const l of lineups.filter((x:any)=>Number(x.team_id)===TEAM_ID)){
      ...
      await supabase.from('match_players').upsert({user_id:user.id,match_id:match.id,player_id:pid,started:isStarter,bench:!isStarter,minutes,user_rating:null},{onConflict:'match_id,player_id'});
