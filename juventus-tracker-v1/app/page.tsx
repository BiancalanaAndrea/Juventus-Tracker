...
function competitors(m:any){const r=m?.raw_provider_json||{};return r?.header?.competitions?.[0]?.competitors||r?.competitions?.[0]?.competitors||[];}
function homeOf(m:any){const c=competitors(m);const j=c.find((x:any)=>String(x?.team?.id||x?.id)==='111'||/juventus/i.test(x?.team?.displayName||x?.displayName||''));return j?.homeAway==='home';}
function actualScores(m:any){const c=competitors(m);const j=c.find((x:any)=>String(x?.team?.id||x?.id)==='111'||/juventus/i.test(x?.team?.displayName||x?.displayName||''));const o=c.find((x:any)=>x!==j);const js=Number(j?.score??j?.currentScore??m.juve_score),os=Number(o?.score??o?.currentScore??m.opponent_score);return {juve:Number.isFinite(js)?js:Number(m.juve_score),opp:Number.isFinite(os)?os:Number(m.opponent_score)};}
