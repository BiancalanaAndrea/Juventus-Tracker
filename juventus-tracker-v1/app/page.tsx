...
const JUVE='https://soccer.highlightly.net/images/teams/juventus.png';
...
function actualScores(m:any){const r=m?.raw_provider_json||{}; const h=r?.homeTeam||r?.home||r?.teams?.home||{}; const a=r?.awayTeam||r?.away||r?.teams?.away||{}; const hj=String(h?.name||'').toLowerCase().includes('juventus'); const juve=hj?h:a,opp=hj?a:h; const js=Number(juve?.score??juve?.currentScore??m.juve_score), os=Number(opp?.score??opp?.currentScore??m.opponent_score); return {juve:Number.isFinite(js)?js:Number(m.juve_score),opp:Number.isFinite(os)?os:Number(m.opponent_score)};}
