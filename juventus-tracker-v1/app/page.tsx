...
export default async function Dashboard() {
  ...
  return <div>
    ...
    <section className="hero-card"><div><span className="pill">{next?'PROSSIMA PARTITA':'ULTIMA PARTITA'}</span><h2>Juventus <span>vs</span> {next?.opponent ?? last?.opponent ?? '—'}</h2><p>{next ? `${next.competition} · ${new Date(next.match_date).toLocaleString('it-IT',{dateStyle:'medium',timeStyle:'short'})}` : last ? `${last.competition} · ${new Date(last.match_date).toLocaleDateString('it-IT')}` : 'Sincronizza Sportmonks dalle Impostazioni'}</p></div><div className="hero-badge">J</div></section>
