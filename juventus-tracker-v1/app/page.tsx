...
function homeOf(m:any){const r=m?.raw_provider_json||{}; const h=r?.homeTeam||r?.home||r?.teams?.home||{}; return String(h?.name||'').toLowerCase().includes('juventus');}
function actualScores(m:any){const js=Number(m?.juve_score),os=Number(m?.opponent_score);return {juve:Number.isFinite(js)?js:0,opp:Number.isFinite(os)?os:0};}
