...
function competitorsOf(m:any){const r=m?.raw_provider_json||{};return r?.header?.competitions?.[0]?.competitors||r?.competitions?.[0]?.competitors||r?.competitors||[]}
function homeOf(m:any){const cs=competitorsOf(m);const j=cs.find((c:any)=>String(c.team?.id)==='111'||/juventus/i.test(String(c.team?.displayName||'')));if(j)return j.homeAway==='home';const r=m?.raw_provider_json||{};const h=r?.homeTeam||r?.home||r?.teams?.home||{};return /juventus/i.test(String(h?.name||h?.displayName||''));}
function actualScores(m:any){const cs=competitorsOf(m);const j=cs.find((c:any)=>String(c.team?.id)==='111'||/juventus/i.test(String(c.team?.displayName||'')));const o=cs.find((c:any)=>c!==j);const js=Number(j?.score?.displayValue??j?.score??j?.displayValue??m.juve_score),os=Number(o?.score?.displayValue??o?.score??o?.displayValue??m.opponent_score);return {juve:Number.isFinite(js)?js:Number(m.juve_score),opp:Number.isFinite(os)?os:Number(m.opponent_score)};}
