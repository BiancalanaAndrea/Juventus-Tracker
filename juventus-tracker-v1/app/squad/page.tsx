...
export default async function Squad() {
 ...
 const {data:players}=await supabase.from('players').select('*').eq('user_id',user.id).eq('active',true);
 const posOrder:any={GK:0,Goalkeeper:0,Portiere:0,DF:1,D:1,Difensore:1,MF:2,M:2,Centrocampista:2,FW:3,F:3,Attaccante:3};
 players?.sort((a:any,b:any)=>{const pa=posOrder[a.position]??9,pb=posOrder[b.position]??9;return pa-pb || (Number(a.number??999)-Number(b.number??999)) || a.name.localeCompare(b.name);});
