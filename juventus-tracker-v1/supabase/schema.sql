...
create table if not exists public.tracker_settings (
 user_id uuid primary key references auth.users(id) on delete cascade, provider text not null default 'sportmonks', season text not null default '2026/27', football_api_key text, updated_at timestamptz not null default now()
);
